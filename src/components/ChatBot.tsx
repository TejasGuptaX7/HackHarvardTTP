...
export default function ChatBot({ onRecommendationsUpdate }: ChatBotProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Notify parent component (e.g., map)
      if (newRecs.length && onRecommendationsUpdate) {
        onRecommendationsUpdate(newRecs);
      }
    } catch (error) {
      console.error(error);
      setMessages((prev) => [
                  ...
                  {msg.recommendations &&
                    msg.recommendations.map((rec) => (
                      <motion.div
                        ...
                        className={`mt-2 border rounded-xl p-3 transition-all ${
                          rec.recommended
